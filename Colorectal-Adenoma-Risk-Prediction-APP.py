...
l_CerealsLegumes = tk.Label(window,   #定义一个label的对象，在window上的label，window是label的主题
                            text = '6. How many cereals do you eat:',   #label上的文字
...
l_Aspirin = tk.Label(window,   #定义一个label的对象，在window上的label，window是label的主题
                    text = '9. Do you take aspirin regularly:',   #label上的文字
...
def submit():    #submit()函数的作用：提取几个对话框里的内容，并转成整数，计算风险
    ...
    n = -6.05 + 0.69*cancerhistory_int + 0.26*sex_int + 0.28*processedmeat_int + 0.61*beverage_int + 0.31*coffee_int + 0.24*smoking_int + 0.2*familyhistory_int + 0.22*sedentary_int + 0.27*hyperlipidemia_int + 0.29*drinking_int + 0.08*age_int - 0.53*cerealslegumes_int - 0.82*vegetablesfruits_int - 1.93*aspirin_int
    z = math.exp(n)
    Z = z / (1+z)
    ...
    m = -1.08 + 0.85*cancerhistory_int + 0.3*processedmeat_int + 0.18*sex_int + 0.58*beverage_int + 0.48*fish_int + 0.35*smoking_int + 0.28*familyhistory_int + 0.3*hyperlipidemia_int + 0.09*hypertension_int - 0.02*moderateactivity_int - 0.47*cerealslegumes_int - 0.75*vegetablesfruits_int
    y = math.exp(m)
    Y = y / (1+y)

    if Y < 0.34:
        relative = absolute + "\nYour relative risk for colorectal adenoma:\nLow risk\n\nThe recommended age for you to start\ncolonoscopy screening is 53 years old"
        var_results.set(relative)
    else:
        relative = absolute + "\nYour relative risk for colorectal adenoma:\nHigh risk\n\nThe recommended age for you to start\ncolonoscopy screening is 42 years old"
        var_results.set(relative)
